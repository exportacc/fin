...
random = pd.DataFrame({
    '2330' : np.random.normal(mu[0],std[0],size=number) , 
    '2882' : np.random.normal(mu[1],std[1],size=number) ,
    '2317' : np.random.normal(mu[2],std[2],size=number)
})
random_custom = np.asarray(random)
...
corr_random = [ cholesky_l @ random_custom[num,:].T for num in range(len(random_custom))] 
